...
- **B7** — Estadísticas agregadas, frase motivacional (con caché) y nutrición (API Ninjas)
...
- `API_NINJAS_KEY`: API key gratuita de [api-ninjas.com](https://api-ninjas.com)
  (solo registro, sin tarjeta) — Nutrition API (para B7/nutrición;
  Nutritionix ya no ofrece capa gratuita y Edamam se reemplazó por esta
  opción más simple de obtener).
...
| POST | `/api/nutrition/log` | JWT | Analiza `queryText` en lenguaje natural con API Ninjas y guarda el registro |
...
- Responde `201` con `calories`/`proteinG`/`carbsG`/`fatG` reales de
  API Ninjas. Requiere `API_NINJAS_KEY` en el `.env`; si falta, si
  API Ninjas falla, o si se alcanza el límite gratuito de la cuenta →
  `502` controlado.
