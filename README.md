...
- **B7** — Estadísticas agregadas, frase motivacional (con caché) y nutrición (Nutritionix)
...
- `NUTRITIONIX_APP_ID` / `NUTRITIONIX_APP_KEY`: credenciales gratuitas de
  [developer.nutritionix.com](https://developer.nutritionix.com) (para B7/nutrición).
...
| POST | `/api/nutrition/log` | JWT | Analiza `queryText` en lenguaje natural con Nutritionix y guarda el registro |
...
- Responde `201` con `calories`/`proteinG`/`carbsG`/`fatG` reales de
  Nutritionix. Requiere `NUTRITIONIX_APP_ID`/`NUTRITIONIX_APP_KEY` en el
  `.env`; si faltan o Nutritionix falla → `502` controlado.
